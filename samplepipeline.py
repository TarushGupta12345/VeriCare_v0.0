...
os.environ["MEDICAL_BILLING_ERRORS"] = os.getenv("MEDICAL_BILLING_ERRORS")
llm = ChatOpenAI(model="gpt-4o")
prompt = """You are an expert in medical billing. Given the following lines of text extracted from a bill,
    identify all errors in the bill."""

def process_bill_image(image_path: str) -> list:
    try:
...
def identify_clerical_errors(bill_lines: list) -> str:
    prompt_clerical = """
...
inpatient vs outpatient - This refers to the classification of a patient's care setting, which directly affects billing codes, reimbursement rates, and coverage.
...
Duplicate Provider Billing - When multiple providers bill for the same service, especially during surgeries (e.g., two surgeons, or a surgeon and assistant, both billing primary CPT).
    ...
    specific error, and you will then list the amount of money that can be saved by correcting this error. In some cases, you may also need to search up the CPT code to get more information on it to see if some procedures
